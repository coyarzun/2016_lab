...
void draw() {
  for (int i=0; i< width; i++) {
    for (int j=0; j< height; j++) {  
      ...
      float r = rr*cos(sin(radians(frameCount))*a)*sin(a);//*cos(a);
      
      set(i, j, color(    
      //fbm(new PVector(r,a),              0.02/1.0),//0.02/4.0),
      //fbm(new PVector(width-r+1,a),      0.02/1.0),
      //fbm(new PVector(map(r,0,rr,0,1),map(a,0,TWO_PI,0,1)),   2)//+frameCount
      fbm(new PVector(r              , a                 ),   .2),
      fbm(new PVector(map(r,0,width/2,0,1),map(a,0,TWO_PI,0,1)),   .02),
