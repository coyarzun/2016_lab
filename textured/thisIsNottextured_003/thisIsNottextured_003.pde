...
void draw(){
  ...
  for(int i=0; i<cols; i++){
    for(int j=0; j<rows; j++){
      pushMatrix();
      rotateY(i*da);
      float radioMayorss = radioMayor*cos(1*j*db);//frameCount*0.0125
      translate(radioMayorss,0,0);
      rotateZ(j*db);
      rotateZ(map((i)%cols,0,cols,0,TWO_PI));
      float radioMenorsss = radioMenor*cos(1*i*da);//frameCount*0.0125
      ...
      fill(0);
      stroke(color(map(i,0,cols,0,255),255,255));//240, 255, 255);
