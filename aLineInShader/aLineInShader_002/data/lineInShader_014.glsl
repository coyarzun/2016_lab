...
void main(void) {
  ...
  vec4 color = texture2D(ppixels, uv);
  ...
  float n = noise(color.r+sin(time*PI/360.0+rm*1.0));

  float stime = n*(1.0+cos(a));//1000.0;//*1000.0;//time*0.01

  vec2 rmx = vec2(r,r);
  float rr,gg,bb;

  rr   = fbm(stime+rm*1.0);
  bb   = pattern(stime+rm*1.0);
  gg   = pattern2(stime*+rm*1.0);  
