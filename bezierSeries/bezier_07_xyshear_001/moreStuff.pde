void doImg(){
  ...
  pg.beginDraw();
  pg.colorMode(HSB);
  pg.background(color(random(255),255,255));
  pg.noStroke();pg.lights();
  pg.translate(width/2,height/2);
  pg.rotateX(radians(frameCount+30));
  pg.rotateY(radians(frameCount+30));
  pg.box(150);
  pg.endDraw();
  //pg.updatePixels();
  
  //arrayCopy(pg.pixels,pimg.pixels);
}
void doNoiseBW(){
