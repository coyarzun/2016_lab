...
void setup() {
  ...
  img = createImage(width, height, RGB);//loadImage("mandy_400x400.jpg");
  uc33 = new UC33();
...
void processImage() {
  ...
  for (int i=0; i<img.pixels.length; i++) {
    ...
    hh+=frameCount*map(uc33.faders[1].normVal, 0, 1, 0, 16); 
    hh%=256;
    s+=frameCount*map(uc33.faders[2].normVal, 0, 1, 0, 16);
    s%=256;
    b+=frameCount*map(uc33.faders[3].normVal, 0, 1, 0, 16);
    b%=256;
    pixels[i] = color(hh, s, b);
  }
  updatePixels();
}
void sideShear() {
